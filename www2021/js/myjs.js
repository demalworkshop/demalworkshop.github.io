function adjust_times_user_time_zone() {
    ...
    $(".workshopitem").each(function (index) {
        var a = ($(this).data("needed"));
        var adjusted_time = moment.tz($(this).data("needed"), "America/Los_Angeles").tz(user_time_zone);
...
function adjust_times_CEST() {
    $(".workshopitem").each(function (index) {
        $(this).text(moment.tz($(this).data("needed"), "America/Los_Angeles").tz("Europe/Paris").format("HH:mm"));
    });
